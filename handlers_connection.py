"""Connection lifecycle for Segment Connector."""
...
@chat.function(
    "connect_segment",
    "Connect Segment account via credentials.",
    action_type="write",
    chain_callable=True,
    event="segment-connector.connect_segment",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_segment(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Segment Connector."""
    client = SegmentClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Segment Account",
...
@chat.function(
    "list_connections",
    "List connected Segment accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_segment",
    "Disconnect Segment account.",
    action_type="write",
    chain_callable=True,
    event="segment-connector.disconnect_segment",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_segment(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
