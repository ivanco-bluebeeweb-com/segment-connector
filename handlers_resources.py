"""Resource handlers for Segment Connector."""
...
@chat.function("list_audiences", "List audiences in Segment.", action_type="read", chain_callable=True, event="segment-connector.list_audiences", effects=["read:audiences"], data_model=AudienceList)
...
@chat.function("get_audience", "Get details of one Audience in Segment.", action_type="read", chain_callable=True, event="segment-connector.get_audience", effects=["read:audience"], data_model=AudienceRecord)
...
@chat.function("audit_audience_health", "Audit health of Segment audiences and connectivity.", action_type="read", chain_callable=True, event="segment-connector.audit_audience_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_audience_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({
            ...
            "summary": f"Segment healthy. Sampled {len(items)} audiences."
        }, summary=f"Segment health check passed with {len(items)} audiences.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Segment health: {e}")
